...
### install haaproxy
WORKDIR /haproxy-src
RUN apk add --no-cache build-base lua5.3-dev linux-headers pcre2-dev libressl-dev
wget http://www.haproxy.org/download/2.0/src/haproxy-2.0.8.tar.gz -O haproxy.tar.gz
tar xvzf haproxy.tar.gz --strip-components=1

make -j 8 all DESTDIR=/output PREFIX=/haproxy 'TARGET=linux-glibc' 'USE_GETADDRINFO=1' 'USE_LUA=1' 'LUA_INC=/usr/include/lua5.3' \
'LUA_LIB=/usr/lib/lua5.3' 'USE_OPENSSL=1' 'USE_PCRE2=1' 'USE_PCRE2_JIT=1' 'USE_ZLIB=1' \
'EXTRA_OBJS=contrib/prometheus-exporter/service-prometheus.o'

make install-bin DESTDIR=/output PREFIX=/haproxy 'TARGET=linux-glibc' 'USE_GETADDRINFO=1' 'USE_LUA=1' \
'LUA_INC=/usr/include/lua5.3' 'LUA_LIB=/usr/lib/lua5.3' 'USE_OPENSSL=1' 'USE_PCRE2=1' 'USE_PCRE2_JIT=1' \
'USE_ZLIB=1' 'EXTRA_OBJS=contrib/prometheus-exporter/service-prometheus.o'
